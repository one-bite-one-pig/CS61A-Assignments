...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self,lower_bound=0,upper_bound=float('inf')):
        ...
        s=self.place
        times=0
        while s and not s.is_hive:
            if len(s.bees)>0 and times>=lower_bound and times<=upper_bound:
                return random_bee(s.bees)
            s=s.entrance
            times+=1
        return None
    ...
    def action(self, gamestate):
        ...
        self.throw_at(self.nearest_bee(self.lower_bound,self.upper_bound))
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        bees_copy = current_place.bees[:] 
        # Call the inherited reduce_health method to reduce the FireAnt's health
        super().reduce_health(amount)
        
        # Calculate the total damage to reflect
        total_damage = amount
        if self.health <= 0:
            total_damage += self.damage    
        # Damage all bees in the place
...
class HungryAnt(Ant):
    ...
    def action(self,gamestate):
        if self.cooldown!=0:
            ...
            attacked=random_bee(self.place.bees)
            if attacked:
                attacked.reduce_health(attacked.health)
